...
export default function ContentBlockRenderCode(props: { block: SupernovaTypes.DocumentationPageBlockRenderCode }) {
    ...
    const onCopyCode = (event: React.SyntheticEvent) => {  
        ...
        event.preventDefault()
        const code = (window as any).sandboxEngine.getCodeForSandboxId(sandboxId)
    ...
    return <div className="sandbox-main-wrapper">
        ...
        <div className="sandbox-state-wrapper">
            ...
            <div className={`sandbox-container ${ showChecker ? "checkered-background-light" : ""}`} style={{ height: props.block.height ?? 200, backgroundColor: backgroundColor}}>
                <div id={ sandboxId } sn-sandbox-data={ props.block.sandboxData }></div>
            </div>
        </div>
        {/* Only show code if the editor allows it */}
        { props.block.showCode ?
        <div className="sandbox-container-meta">
    
            <div className="sandbox-container-buttons">
            {/* Toggle code button */}
            <a className="btn btn-light" data-toggle="collapse" href={`#codepreview-${sandboxId}`} role="button" aria-expanded="false" aria-controls="collapseExample" onClick={onToggleCode}>
                Toggle code
            </a>
    
            {/* Copy code button */}
            <a className="btn btn-light sandbox-copy" data-toggle="tooltip" title="Copy component code" onClick={onCopyCode}>
                <div className="sandbox-copy-icon" data-toggle="copy-from-sandbox" data-target={`${sandboxId}`}></div>
            </a>
            </div>
    
            {/* Rendered code */}
            <div className={`sandbox-text ${!showCode ? "collapse" : ""}`} id={`codepreview-${sandboxId}`}>
