...
export default function TokenPreviewSmall(props: { token: SupernovaTypes.Token }) {
  switch (props.token.tokenType) {
    case SupernovaTypes.TokenType.Blur: {
      ...
      switch (typedToken.value.type) {
        case SupernovaTypes.BlurType.background:
          return (
            <div className="token-preview-container checkered-background">
              <div
                className="token-preview-container-blur"
    ...
    case SupernovaTypes.TokenType.Border: {
      ...
      return (
        <div className="token-preview-container">
          <div
            className="token-preview-container-border"
    ...
    case SupernovaTypes.TokenType.Radius: {
      ...
      return <div className="token-preview-container" style={{ borderRadius: typedToken.value.radius.measure }}></div>
    }
    case SupernovaTypes.TokenType.Shadow: {
      let typedToken = props.token as SupernovaTypes.ShadowToken
      return (
        <div className="token-preview-container">
          <div style={{ width: 20, height: 20, boxShadow: UtilFormatting.scaledCSSShadow(typedToken, 4) }}></div>
        </div>
      )
    }
