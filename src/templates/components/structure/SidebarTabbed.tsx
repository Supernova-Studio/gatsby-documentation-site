...
export default function SidebarTabbed(props: { pageId: string; groupId: string | null }): ReactElement<any, any> {
  ...
  return (
    <>
      ...
      {isGeneratingRootElement ? rootGroup ? <SidebarHeader item={rootGroup} /> : <SidebarHeader item={page} /> : null}
          ...
          if (c.itemType === "Page") {
            ...
            return (
              <li className="nav-item" id={c.id}>
                ...
                {isActive ? <SidebarTabbed pageId={props.pageId} groupId={c.id} /> : null}
              </li>
            )
          }
        })}
...
function SidebarHeader(props: { item: SupernovaTypes.DocumentationItem }) {
  return (
    <div className="nav-name">
      <span className="typographyDocsViewLabelCaps">{props.item.title?.toUpperCase()}</span>
    </div>
  )
}
