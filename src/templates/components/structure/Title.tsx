...
export default function Title(props: { pageId: string }) {
  ...
  if (header.backgroundImageAssetUrl) {
    ...
    return (
      <>
        <div
          className={`container-title ${centeredClass} ${flexedClass}`}
          style={{
  ...
  return (
    <>
      ...
      <div className="slider">
        ...
        {props.header.showCoverText ? (
          <h1 style={{ color: props.header.foregroundColor ? props.header.foregroundColor : undefined }}>{props.title}</h1>
        ) : null}
        {props.header.description ? (
          <p className="subtitle" style={{ color: props.header.foregroundColor ? props.header.foregroundColor : undefined }}>
            {props.header.description}
          </p>
        ) : null}
      </div>
      ...
      <div className="container-tabs">
        <div className="tabbed-navigation">
          <ul className="nav nav-tabs" id="page-tab" role="tablist">
            {pages.map((p) => {
              return (
                <li className="nav-item" key={`tab-${p.id}`}>
                  <a
                    className={`nav-link ${props.page.id === p.id ? "active" : ""}`}
                    id={`tab-${p.id}`}
                    href={p.slug}
                    role="tab"
                    aria-controls="home"
